            ...
            <div class="card">
                <div class="card-header">
					<div class="float-left">
						{{ __('Create New Sku Code Details*') }}
					</div>
                ...
                <div class="card-body">
					...
					<form method="POST" action="{{ route('skudetails.store') }}">
					@csrf
								...
								<div class="form-group">
                                    ...
                                    <select class="form-control" id="mplace" name="mplace" autofocus>
										...
										@foreach ($marketplaces as $key => $val)
											<option value="{{ $val->id }}" {{ $val == old('mplace') ? 'selected' : '' }}>{{ $val->name }}</option>
										@endforeach
									</select>
								</div>
							</div>
								...
								<div class="form-group">
                                    ...
                                    <select class="form-control" id="warehouse" name="warehouse" autofocus>
										...
										@foreach ($warehouses as $key => $val)
											<option value="{{ $val->id }}" {{ $val == old('warehouse') ? 'selected' : '' }}>{{ $val->name }}</option>
										@endforeach
									</select>
								</div>
							</div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                            <div class="form-group">
									<label for="pgkwght" class="col-form-label text-md-right">{{ __('Pkg-Wgtk(kg)*') }}</label>
