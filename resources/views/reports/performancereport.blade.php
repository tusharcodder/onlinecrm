    ...
    <div class="row justify-content-center mb-2">
		<div class="col-md-12">
			<div class="card">
				...
				<div class="card-body">
					<form action="{{ route('searchperformancereport') }}" id="searchform" name="searchform" method="GET">
						@csrf
						<div class="row">							
							<div class="col-md-3">
						...
						<div class="row">
							...
							<div class="col-md-3">
								<div class="form-group">
									<label for="percategory" class="col-form-label text-md-right">{{ __('Performance category*') }}</label>
									<select class="form-control" id="percategory" name="percategory">
										<option value="">-- Select --</option>
										@foreach ($cat as $key => $val)
											<option value="{{ $val }}" {{ $val == $percat ? 'selected' : '' }}>{{ $val }}</option>
										@endforeach
									</select>
								</div>
							</div>							
						...
						<div class="form-group row mb-1">
                            <div class="col-md-12">
								...
								@if(!empty($request->input()))
									<a href="{{ route('performancereport') }}" class="btn btn-info">{{ __('Reset') }}
									</a>
								@endif
                            </div>
                        </div>
	...
	<div class="row justify-content-center">
		<div class="col-md-12">
			<div class="card">
				...
				<div class="card-body">
					@can('downloadperformancereport')
						<form action="{{ route('downloadperformancereport') }}" method="POST">
						@csrf
						...
						<table class="table table-bordered">
							...
							@if($stockreports->total() > 0)
								...
								@php
									...
									$textclass = "";
									if($stock->performance == 'Fast')
										$textclass = "btn btn-success btn-sm";
									elseif($stock->performance == 'Slow')
										$textclass = "btn btn-danger btn-sm";
									else	
										$textclass = "btn btn-warning btn-sm";
								@endphp
								...
								<tr>
									...
									<td>{{ $salethrough }}%</td>
									<td><b class="{{ $textclass }}" style="font-size: 14px;">{{ $stock->performance }}</b></td>
									<td>
