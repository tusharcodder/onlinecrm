    ...
    */
    ...
    {	
		...
		if($format == "withheading"){ // data with heading
			...
			$query = SkuDetail::select('skudetails.*','market_places.name as Market_Place')
			->join("market_places","market_places.id","=","skudetails.market_id");//->join("warehouses","warehouses.id","=","skudetails.warehouse_id");			
			...
			$results = $query->orderBy('skudetails.market_id', 'ASC')->get();
